	...
	const projectDetails = [
		{
			...
			startDate: '2021',
			description:
				'The purpose of this project is to develop independent library that can be “wired into” IHS Markit existing web applications (like AMS, PDM etc) natively at a code/API level to get professional visualizations and dashboards added natively. Existing Tableau portal to view/download visualization for daily sales data in separate implementation instead integration to Auto Market Signals (AMS) application. However, Tableau itself running as separate application so for any visualization user must switch to this application from its current application, which is bringing in distraction to the user. ',
			technology:
				'ReactJs, JSON React Layouts, Redux toolkit, ES6+, HTML5, Material UI, Git Lab, NPM',
			demoLink: '',
			githubLink: '',
			client: 'IHS Markit',
			images: [
